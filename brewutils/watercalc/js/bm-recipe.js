var RecipeDisplay = {
    ...
    show: function(recipe) {
        ...
        $(this.div).find(".boil-time").text(recipe.boilTime + this.u_min);
        ...
        $(this.div).find(".batch-size").text(bs + (opt.metric ? this.u_l : this.u_gal));
        if (recipe.style)
            $(this.div).find(".recipe-style").text(recipe.style.comName);

        var og = recipe.getValue("og");

        if (og) $(this.div).find(".recipe-og").html((opt.plato) ? (BrewMath.sg2plato(og).toFixed(1) + this.u_p) : og.toFixed(3));
        var fg = recipe.getValue("fg");
        if (fg) $(this.div).find(".recipe-fg").html((opt.plato) ? (BrewMath.sg2plato(fg).toFixed(1) + this.u_p) : fg.toFixed(3));
    ...
    showFermentables: function(ferms) {
        ...
        $.each(ferms, function(k, v) {
            ...
            tr.find("td.ferm-weight").text(amt.toFixed(2) +
                (t.opt.metric ? t.u_kg : t.u_lb));
            tr.find("td.ferm-yield").text(t.ppg ? Math.round(46.177 * v.yield / 100) : (v.yield + "%"));
            tr.find("td.ferm-color").text(v.color + t.u_Lovibond);
    ...
    showSameTimeHop: function(hops, head, type, startorder) {
        ...
        $.each(groups, function(i, row) {
            ...
            $.each(row, function(i, h) {
                ...
                if (rs) {
                    rs = false;
                    tr.find("td.hop-time").attr("rowspan", row.length).text(row[0].time + t.u_min);
                ...
                tr.find("td.hop-weight").text(am.toFixed(2) + (t.opt.metric ? t.u_g : t.u_oz));
    ...
    showHopMisc: function(hops, misc) {
        ...
        if (aroma.length > 0) {
            ...
            $.each(aromaGroupByTemperature, function(i, group) {
                ...
                // not yet sorted, session.find(".hopstand-time").text("" + group[0].time + t.u_min);
                var starttemp = group[0].temperature;
                var keeptemp = group[0].standtemperature;
                if (t.celius) {
                    session.find(".hopstand-stemp").html(starttemp.toFixed(1) + t.u_c);
                    session.find(".hopstand-ktemp").html(keeptemp.toFixed(1) + t.u_c);
                } else {
                    session.find(".hopstand-stemp").html(t.toFahrenheit(starttemp).toFixed(1) + t.u_f);
                    session.find(".hopstand-ktemp").html(t.toFahrenheit(keeptemp).toFixed(1) + t.u_f);
                }
                head.after(session);
                head = session;
                ...
                session.find(".hopstand-time").text("" + hopstandhop[0] + t.u_min);
    ...
    showYeasts: function(yeasts) {
        ...
        $.each(yeasts, function(k, v) {
            ...
            tr.find(".yeast-temp").html(min.toFixed(1) + " ~ " + max.toFixed(1) + (t.celius ? t.u_c : t.u_f));
    ...
    showFermetationStage: function(stages) {
        ...
        $.each(stages, function(k, v) {
            ...
            tr.find(".ferm-temp").html(tmp.toFixed(1) + (t.celius ? t.u_c : t.u_f));
    ...
    showMash: function(mashes) {
        ...
        $.each(mashes, function(k, v) {
            ...
            var tmp = t.celius ? v.temp : t.toFahrenheit(v.temp);
            tr.find("td.mash-temp").html(tmp.toFixed(1) + (t.celius ? t.u_c : t.u_f));

            t.bmAuto.rest_tp.push(v.temp);
            if (!isNaN(v.time)) {
                tr.find("td.mash-time").text(v.time + t.u_min);
            ...
            if (v.amount > 0) {
                ...
                tr.find("td.mash-water").text(amt.toFixed(2) + (t.opt.metric ? t.u_l : t.u_qt));
            }
            head.after(tr);
            head = tr;
        });
...
var Recipes = {
    ...
    dispPreference: function() {
        ...
        $("#kettlemass_unit").html(Recipes.userPreference.metric ? "L" : "qt");
